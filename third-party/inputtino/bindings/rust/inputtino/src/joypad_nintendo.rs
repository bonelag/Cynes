...
use crate::sys::{
    inputtino_joypad_switch_create, inputtino_joypad_switch_destroy,
    inputtino_joypad_switch_get_nodes, inputtino_joypad_switch_set_on_rumble,
    inputtino_joypad_switch_set_pressed_buttons, inputtino_joypad_switch_set_stick,
...
impl SwitchJoypad {
    ...
    pub fn new(device: &DeviceDefinition) -> Result<Self, InputtinoError> {
        make_device(inputtino_joypad_switch_create, device).map(|joypad| SwitchJoypad {
            joypad,
